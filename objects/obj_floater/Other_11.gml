...
if (!activated) {
	...
	var max_angle_change = 6, dir_difference = angle_difference(new_dir, direction);
	...
	// Move in direction
	if (x != player.x) { x += lengthdir_x(2, direction); }
	if (y != player.y) { y += lengthdir_y(2, direction); }
	if (get_coin_flip()) { 
