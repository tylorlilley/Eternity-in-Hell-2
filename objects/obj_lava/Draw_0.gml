...
for (var quadrant = 0; quadrant < 4; quadrant++) {
	...
	if (global.lava_edge_type != lava_edge_types.none) {
		var x_pos = get_quadrant_x_pos(quadrant), y_pos = get_quadrant_y_pos(quadrant);
		if (quadrant == 0 && lava_edge_visible[quadrant][directions.up] && lava_edge_visible[quadrant][directions.left]) { draw_sprite_ext(spr_lava_corners,  lava_edge_image_indexes[quadrant][directions.up], x_pos-4, y_pos-4, 1, 1, 0, bg_color, 1); }
		if (quadrant == 1 && lava_edge_visible[quadrant][directions.up] && lava_edge_visible[quadrant][directions.right]) { draw_sprite_ext(spr_lava_corners, lava_edge_image_indexes[quadrant][directions.right], x_pos+4, y_pos-4, 1, 1, 270, bg_color, 1); }
		if (quadrant == 2 && lava_edge_visible[quadrant][directions.down] && lava_edge_visible[quadrant][directions.left]) { draw_sprite_ext(spr_lava_corners, lava_edge_image_indexes[quadrant][directions.down], x_pos-4, y_pos+4, 1, 1, 90, bg_color, 1); }
		if (quadrant == 3 && lava_edge_visible[quadrant][directions.down] && lava_edge_visible[quadrant][directions.right]) { draw_sprite_ext(spr_lava_corners, lava_edge_image_indexes[quadrant][directions.left], x_pos+4, y_pos+4, 1, 1, 180, bg_color, 1); }
		/*
