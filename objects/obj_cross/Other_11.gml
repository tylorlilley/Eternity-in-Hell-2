...
if (is_instance_at_coordinates(x, y, player)) {
	with (player) {
		var heart = get_carried_item(obj_heart);
		if (instance_exists(heart) && are_all_collectables_collected()) { 
			...
			if (heart.special) {
				global.controller.evaluation_manager.increment_evaluation_variable("special_heart_victory");
			}
