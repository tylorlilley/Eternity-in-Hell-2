if (process_this_frame()) {
	...
	if (is_instance_at_coordinates(x, y, global.player)) {
		with (global.player) {
			if (is_carrying_item(obj_heart) && game_progress_has_been_completed()) { global.controller.completion_amount += 1; play_sound(snd_win, false); instance_destroy(other.id); }
