...
function is_current_map_possible() {
	...
	do {
		...
		if (array_length(unlocked_exit_lists_to_verify) > MAX_WALKING_DEPTH) { map_is_possible = false; }
	...
	while (array_length(exits_to_walk_through) > 0) {
		...
		if (!array_contains(visited_rooms, target_room)) {
			...
			if (!chosen_room.locked_exits[chosen_dir] || array_contains(unlocked_exits, chosen_room.locked_exits[chosen_dir])) {
				...
				if (target_room.has_keys > 0) { keys_found += (target_room.has_special_item && target_room.chest_obj == obj_key) ? 9999 : 1; } //if (keys_found >= 9999) { show_debug_message("SPECIAL KEY FOUND ON WALK"); } }
