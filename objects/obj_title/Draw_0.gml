var game_manager = global.game_manager, game_color = get_game_color();
var run_count_string = (global.is_test_mode) ? "Runs Attempted: 99999" : get_run_count_string(global.difficulty);
var death_count_string = (global.is_test_mode) ? "Deaths: 9999" : get_death_count_string(global.difficulty), win_count_string = (global.is_test_mode) ? "Victories: 9999" : get_win_count_string(global.difficulty);
var title_y_pos = room_height*2, title_scale = 0.125, blink = is_blink_frame();

// Draw General Submenu Info
draw_set_color(c_white);
draw_set_font(ft_hud);
draw_set_valign(fa_middle);
draw_set_halign(fa_center);
var submenu_title = "";

if (prepare_screen) { submenu_title = "Prepare Yourself"; }
else if (options_screen) { submenu_title = "Options"; }
else if (controls_screen) { submenu_title = "View Controls"; }
else if (death_log_screen) { submenu_title = "View Death Log"; }
if (options_screen || controls_screen || death_log_screen || evaluation_log_screen || prepare_screen) {
	draw_set_font(ft_hud);
	if (!death_log_screen && !evaluation_log_screen) { draw_text(room_width/2, 8+15, submenu_title); }
	
	draw_set_font(ft_hud_small);
	var return_text = get_input_x_key_string() + ": Return", return_y_pos = room_height-4, return_x_pos = room_width/2;
	draw_set_color(c_white);
	
	if (death_log_screen) { draw_text(return_x_pos, return_y_pos-10, get_input_z_key_string() + ": Change Sort"); }
	else if (options_screen) { draw_text(return_x_pos, return_y_pos-10, get_input_z_key_string() + ": Select Option"); }
	else if (prepare_screen) { draw_text(return_x_pos, return_y_pos-10, get_input_z_key_string() + ": Begin"); }
	else { return_y_pos -= 4; }
	
	draw_set_color(game_color);
	draw_text(return_x_pos, return_y_pos, return_text);
	draw_set_font(ft_hud);
}
draw_set_color(c_white);

// Draw Border
if (!loading) {
	var main_menu = (!prepare_screen && !options_screen && !death_log_screen && !controls_screen && !evaluation_log_screen), wall_sprite_color = merge_color(c_white, c_black, 0.5);
	for (var border_x_pos = -8; border_x_pos < room_width+16; border_x_pos += 16;) {
		draw_sprite_ext(spr_wall, 0, border_x_pos, 8, 1, 1, 0, wall_sprite_color, 1);
		//draw_sprite_ext(spr_wall, 0, border_x_pos, room_height-16, 1, 1, 0, wall_sprite_color, 1);
		//draw_sprite_ext(spr_wall, 0, border_x_pos, room_height-8, 1, 1, 0, wall_sprite_color, 1);
		var gap_width = (global.input == inputs.gamepad && !main_menu) ? 64 : 40;
		if (death_log_screen || options_screen || prepare_screen) { gap_width += 16; }
		if (border_x_pos < room_width/2 - gap_width || border_x_pos > room_width/2 + gap_width) { draw_sprite_ext(spr_wall, 0, border_x_pos, room_height-8, 1, 1, 0, wall_sprite_color, 1); }
		
		if (main_menu) {
			draw_sprite_ext(spr_wall, 0, border_x_pos, 8+(16*8), 1, 1, 0, wall_sprite_color, 1);
			if (global.seed_option != seed_options.specified || border_x_pos < room_width/2 - 48 || border_x_pos > room_width/2 + 48) {  draw_sprite_ext(spr_wall, 0, border_x_pos, room_height-8-(16*5), 1, 1, 0, wall_sprite_color, 1); }
			//draw_sprite_ext(spr_wall, 0, border_x_pos, 8+16, 1, 1, 0, wall_sprite_color, 1);
		}
		else if (death_log_screen) {
			draw_sprite_ext(spr_wall, 0, border_x_pos, 8+(16*2), 1, 1, 0, wall_sprite_color, 1);
			draw_sprite_ext(spr_wall, 0, border_x_pos, 8+(16*5), 1, 1, 0, wall_sprite_color, 1);
			//draw_sprite_ext(spr_wall, 0, border_x_pos, room_height-(16*3), 1, 1, 0, wall_sprite_color, 1);
		}
		else if (prepare_screen) {
			draw_sprite_ext(spr_wall, 0, border_x_pos, 8+(16*3), 1, 1, 0, wall_sprite_color, 1);
		}
		else {
			draw_sprite_ext(spr_wall, 0, border_x_pos, 8+(16*2), 1, 1, 0, wall_sprite_color, 1);
		}
	}
	for (var border_y_pos = -8; border_y_pos < room_height+16; border_y_pos += 16;) {
		draw_sprite_ext(spr_wall, 0, 8, border_y_pos, 1, 1, 0, wall_sprite_color, 1);
		if (main_menu && (border_y_pos < 16*1 || border_y_pos > 16*9)) { 
			draw_sprite_ext(spr_wall, 0, 8+16, border_y_pos, 1, 1, 0, wall_sprite_color, 1);
			if (border_y_pos < room_height-16) { draw_sprite_ext(spr_wall, 0, room_width-8-16, border_y_pos, 1, 1, 0, wall_sprite_color, 1); }
		}
		else if (death_log_screen && border_y_pos > 8+(16*5) && border_y_pos < room_height-(16*2)) {
			draw_sprite_ext(spr_wall, 0, -8+(16*4), border_y_pos, 1, 1, 0, wall_sprite_color, 1);
			draw_sprite_ext(spr_wall, 0, -8+(16*7), border_y_pos, 1, 1, 0, wall_sprite_color, 1);
			draw_sprite_ext(spr_wall, 0, -8+(16*11), border_y_pos, 1, 1, 0, wall_sprite_color, 1);
		}
		draw_sprite_ext(spr_wall, 0, room_width-8, border_y_pos, 1, 1, 0, wall_sprite_color, 1);
	}
}


// Draw Menu Specific Stuff
if (loading) { title_y_pos = room_height*2; title_scale = 0.25; }
else if (prepare_screen) {
	var player_x_pos = room_width/2, left_hand_x_pos = player_x_pos - (16*4), right_hand_x_pos = player_x_pos + (16*4);
	var player_y_pos = (room_height/2)+16;
	
	// Draw Text
	draw_set_valign(fa_middle);
	draw_set_halign(fa_center);
	draw_set_font(ft_hud);
	draw_text(room_width/2, 40, get_difficulty_string(global.difficulty));
	draw_set_font(ft_hud);
	//draw_text(room_width/2, room_height-32, get_input_z_key_string() + ": Begin");
	var best_count = array_length(hand_options);
	var possible_count = array_length(global.available_items[global.difficulty]);
	//var available_text = "AVAILABLE: "+string(best_count)+"/"+string(possible_count);
	//if (best_count == possible_count) { available_text += "; COMPLETE"; draw_set_color(game_color); }
	//var available_text = (best_count == possible_count) ? "COMPLETE" : "";
	//draw_text(room_width/2, 64, available_text);
	draw_set_color(c_white);
	
	// Draw Player Visuals
	if (best_count == possible_count) { draw_sprite_ext(spr_crown, (get_win_count(global.difficulty, graphics_modes.unknown) > 0) ? 1 : 0, player_x_pos, player_y_pos - (16*3), 3, 3, 0, c_white, 1); }
	draw_sprite_ext(spr_player, 1, player_x_pos, player_y_pos, 4, 4, 0, c_white, 1);
	if (global.graphics_mode == graphics_modes.farmer) { draw_sprite_ext(spr_player_farmer, 1, player_x_pos, player_y_pos, 4, 4, 0, c_white, 1); }
	//player_y_pos -= 8;
	draw_set_color(game_color);
	draw_rectangle(left_hand_x_pos-24, player_y_pos-24, left_hand_x_pos+24, player_y_pos+24, false);
	draw_rectangle(right_hand_x_pos-24, player_y_pos-24, right_hand_x_pos+24, player_y_pos+24, false);
	draw_set_color(global.bg_color);
	draw_rectangle(left_hand_x_pos-20, player_y_pos-20, left_hand_x_pos+20, player_y_pos+20, false);
	draw_rectangle(right_hand_x_pos-20, player_y_pos-20, right_hand_x_pos+20, player_y_pos+20, false);
	draw_set_color(c_white);
	if (left_hand_selected && blink) {
		draw_sprite_ext(spr_menu_arrow, 0, left_hand_x_pos + 8, player_y_pos - (16*3) + 4, 2, 2, 90, c_white, 1);
		draw_sprite_ext(spr_menu_arrow, 0, left_hand_x_pos + 8, player_y_pos + (16*3) - 4, 2, 2, -90, c_white, 1);
	}
	else if (!left_hand_selected && blink) {
		draw_sprite_ext(spr_menu_arrow, 0, right_hand_x_pos - 8, player_y_pos - (16*3) + 4, -2, 2, -90, c_white, 1);
		draw_sprite_ext(spr_menu_arrow, 0, right_hand_x_pos - 8, player_y_pos + (16*3) - 4, -2, 2, 90, c_white, 1);
	}
	
	if (left_hand_pos != -1) {
		var item = hand_options[left_hand_pos], sprite_to_use = get_sprite_to_use(object_get_sprite(item), true);
		var item_y_offset = get_carried_item_draw_y_offset(sprite_to_use);
		draw_sprite_ext(sprite_to_use, 0, left_hand_x_pos, player_y_pos+(4*item_y_offset), 4, 4, 0, c_white, 1);
		if (sprite_to_use == spr_compass) { draw_sprite_ext(spr_compass_hands, 0, left_hand_x_pos, player_y_pos, 4, 4, 0, c_white, 1); }
		else if (sprite_to_use == spr_clock) { draw_sprite_ext(spr_clock_sand, 0, left_hand_x_pos, player_y_pos, 4, 4, 0, c_white, 1); }
		else if (sprite_to_use == spr_clock_farmer) { draw_sprite_ext(spr_clock_sand_farmer, 0, left_hand_x_pos, player_y_pos, 4, 4, 0, c_white, 1); }
	}
	if (right_hand_pos != -1) {
		var item = hand_options[right_hand_pos], sprite_to_use = get_sprite_to_use(object_get_sprite(item), true)
		var item_y_offset = get_carried_item_draw_y_offset(sprite_to_use);
		draw_sprite_ext(sprite_to_use, 0, right_hand_x_pos, player_y_pos+(4*item_y_offset), -4, 4, 0, c_white, 1);
		if (sprite_to_use == spr_compass) { draw_sprite_ext(spr_compass_hands, 0, right_hand_x_pos, player_y_pos, -4, 4, 0, c_white, 1); }
		else if (sprite_to_use == spr_clock) { draw_sprite_ext(spr_clock_sand, 0, right_hand_x_pos, player_y_pos, -4, 4, 0, c_white, 1); }
		else if (sprite_to_use == spr_clock_farmer) { draw_sprite_ext(spr_clock_sand_farmer, 0, right_hand_x_pos, player_y_pos, -4, 4, 0, c_white, 1); }
	}
}
else if (options_screen) {
	draw_set_valign(fa_top);
	draw_set_halign(fa_left);
	draw_set_font(ft_hud_small);
	
	var y_pos = 64, x_pos = room_width-64, y_offset = 14, text_x_pos = 32;
	
	// Draw Fullscreen Option
	if (blink && options_pos == 0 && !option_selected) {  draw_sprite_ext(spr_menu_arrow, 0, text_x_pos-8, y_pos+6, 1, 1, 0, c_white, 1); }
	var is_full_screen = global.fullscreen;
	draw_set_halign(fa_left);
	draw_text(text_x_pos, y_pos, "Fullscreen: ");
	draw_set_halign(fa_center);
	draw_text(x_pos, y_pos, ((is_full_screen) ? "ON" : "OFF"));
	if (blink && options_pos == 0 && option_selected) {
		if (is_full_screen) { draw_sprite_ext(spr_menu_arrow, 0, x_pos+16, y_pos+6, 1, 1, 0, c_white, 1); }
		else { draw_sprite_ext(spr_menu_arrow, 0, x_pos-16, y_pos+6, -1, -1, 0, c_white, 1); }
	}
	
	// Draw Screen Scale or Borderless Option
	y_pos += y_offset;
	if (blink && options_pos == 1 && !option_selected) {  draw_sprite_ext(spr_menu_arrow, 0, text_x_pos-8, y_pos+6, 1, 1, 0, c_white, 1); }
	if (!global.fullscreen || os_type != os_windows) {
		draw_set_halign(fa_left);
		draw_text(text_x_pos, y_pos, "Window Size: ");
		draw_set_halign(fa_center);
		draw_text(x_pos, y_pos, "x "+ string(global.window_scaling));
		if (blink && options_pos == 1 && option_selected) {
			if (global.window_scaling < global.max_window_scaling) { draw_sprite_ext(spr_menu_arrow, 0, x_pos+16, y_pos+6, 1, 1, 0, c_white, 1); }
			if (global.window_scaling > 1) { draw_sprite_ext(spr_menu_arrow, 0, x_pos-16, y_pos+6, -1, 1, 0, c_white, 1); }
		}
	}
	else {
		draw_set_halign(fa_left);
		draw_text(text_x_pos, y_pos, "Window Border: ");
		draw_set_halign(fa_center);
		draw_text(x_pos, y_pos, ((global.window_border) ? "ON" : "OFF"));
		if (blink && options_pos == 1 && option_selected) {
			if (global.window_border) { draw_sprite_ext(spr_menu_arrow, 0, x_pos+16, y_pos+6, 1, 1, 0, c_white, 1); }
			else { draw_sprite_ext(spr_menu_arrow, 0, x_pos-16, y_pos+6, -1, -1, 0, c_white, 1); }
		}
	}
	
	// Draw Controls Option
	y_pos += y_offset;
	if (blink && options_pos == 2 && !option_selected) {  draw_sprite_ext(spr_menu_arrow, 0, text_x_pos-8, y_pos+6, 1, 1, 0, c_white, 1); }
	draw_set_halign(fa_left);
	draw_text(text_x_pos, y_pos, "Controls: ");
	draw_set_halign(fa_center);
	draw_text(x_pos, y_pos, get_input_string());
	if (blink && options_pos == 2 && option_selected) {
		if (global.input < 1 || (global.input == 1 && gamepad_is_connected(global.gamepad))) { draw_sprite_ext(spr_menu_arrow, 0, x_pos+40, y_pos+6, 1, 1, 0, c_white, 1); }
		if (global.input > 0) { draw_sprite_ext(spr_menu_arrow, 0, x_pos-40, y_pos+6, -1, 1, 0, c_white, 1); }
	}
	
	// Draw Player Outline
	y_pos += y_offset;
	if (blink && options_pos == 3 && !option_selected) {  draw_sprite_ext(spr_menu_arrow, 0, text_x_pos-8, y_pos+6, 1, 1, 0, c_white, 1); }
	draw_set_halign(fa_left);
	draw_text(text_x_pos, y_pos, "Player Outline: ");
	draw_set_halign(fa_center);
	draw_text(x_pos, y_pos, ((global.player_outline) ? "ON" : "OFF"));
	if (blink && options_pos == 3 && option_selected) {
		if (!global.player_outline) { draw_sprite_ext(spr_menu_arrow, 0, x_pos-16, y_pos+6, -1, 1, 0, c_white, 1); }
		else { draw_sprite_ext(spr_menu_arrow, 0, x_pos+16, y_pos+6, 1, 1, 0, c_white, 1); }
	}
	
	// Draw Screen Flash Option
	y_pos += y_offset;
	if (blink && options_pos == 4 && !option_selected) {  draw_sprite_ext(spr_menu_arrow, 0, text_x_pos-8, y_pos+6, 1, 1, 0, c_white, 1); }
	draw_set_halign(fa_left);
	draw_text(text_x_pos, y_pos, "Screen Flash: ");
	draw_set_halign(fa_center);
	draw_text(x_pos, y_pos, ((global.can_screen_flash) ? "ON" : "OFF"));
	if (blink && options_pos == 4 && option_selected) {
		if (!global.can_screen_flash) { draw_sprite_ext(spr_menu_arrow, 0, x_pos-16, y_pos+6, -1, 1, 0, c_white, 1); }
		else { draw_sprite_ext(spr_menu_arrow, 0, x_pos+16, y_pos+6, 1, 1, 0, c_white, 1); }
	}
	
	// Draw Color Option
	x_pos -= 16;
	y_pos += y_offset;
	if (blink && options_pos == 5 && !option_selected) {  draw_sprite_ext(spr_menu_arrow, 0, text_x_pos-8, y_pos+6, 1, 1, 0, c_white, 1); }
	draw_set_halign(fa_left);
	draw_text(text_x_pos, y_pos, "Color: ");
	draw_set_halign(fa_center);
	var new_color = get_gms_color_from_hex_string(global.game_color_string), underscore_string = " ";
	for (var i = 0; i < 6; i++) {
		underscore_string += (i == color_options_pos) ? "_" : " ";
	}
	if (blink && options_pos == 5 && option_selected) { draw_text(x_pos, y_pos+2, underscore_string); }
	draw_text(x_pos, y_pos, "#"+global.game_color_string);
	draw_sprite_ext(spr_box, 0, 216, y_pos+6, 0.875, 0.875, 0, new_color, 1);
	
	// Draw Minimum Fade Option
	x_pos -= 4;
	y_pos += y_offset*2;
	if (blink && options_pos == 6 && !option_selected) {  draw_sprite_ext(spr_menu_arrow, 0, text_x_pos-8, y_pos+6-y_offset/2, 1, 1, 0, c_white, 1); }
	draw_set_halign(fa_left);
	draw_text(text_x_pos, y_pos-y_offset, "Minimum\nBrightness: ");
	draw_set_halign(fa_center);
	draw_text(x_pos, y_pos-y_offset/2, get_percentage_string(global.game_color_fade));
	if (blink && options_pos == 6 && option_selected) {
		if (global.game_color_fade < 100) { draw_sprite_ext(spr_menu_arrow, 0, x_pos+60, y_pos+6-y_offset/2, 1, 1, 0, c_white, 1); }
		if (global.game_color_fade > 0) { draw_sprite_ext(spr_menu_arrow, 0, x_pos-32, y_pos+6-y_offset/2, -1, 1, 0, c_white, 1); }
	}
	var new_color_minimum_fade = make_color_rgb(global.game_color_fade/100.0 * color_get_red(new_color), global.game_color_fade/100.0 * color_get_green(new_color), global.game_color_fade/100.0 * color_get_blue(new_color));
	//draw_sprite_ext(spr_box, 0, x_pos + 56, y_pos+6, 1, 1, 0, c_white, 1);
	draw_sprite_ext(spr_box, 0, 216, y_pos+6-y_offset/2, 0.875, 0.875, 0, new_color_minimum_fade, 1);
	
	// Draw Lava Edge Type Option
	//x_pos -= 16;
	y_pos += y_offset;
	if (blink && options_pos == 7 && !option_selected) {  draw_sprite_ext(spr_menu_arrow, 0, text_x_pos-8, y_pos+6, 1, 1, 0, c_white, 1); }
	draw_set_halign(fa_left);
	draw_text(text_x_pos, y_pos, "Lava Edge: ");
	draw_set_halign(fa_center);
	draw_text(x_pos, y_pos-y_offset/2, get_lava_edge_type_string());
	if (blink && options_pos == 7 && option_selected) {
		if (global.lava_edge_type < lava_edge_types.wavy_animated) { draw_sprite_ext(spr_menu_arrow, 0, x_pos+80-16-4, y_pos+6, 1, 1, 0, c_white, 1); }
		if (global.lava_edge_type > lava_edge_types.none) { draw_sprite_ext(spr_menu_arrow, 0, x_pos-32, y_pos+6, -1, -1, 0, c_white, 1); }
	}
	
	// Draw Reset to Defaults Option
	y_pos += y_offset*2;
	if (blink && options_pos == 8 && !option_selected) {  draw_sprite_ext(spr_menu_arrow, 0, text_x_pos-8, y_pos+6, 1, 1, 0, c_white, 1); }
	draw_set_halign(fa_center);
	draw_set_color(game_color);
	draw_text(room_width/2, y_pos, "Reset to Default Settings");
	draw_set_color(c_white);
	draw_set_halign(fa_center);
}
else if (controls_screen) {
	draw_set_valign(fa_middle);
	draw_set_halign(fa_left);
	var title_y_pos = room_height*2;
	var x_pos = room_width/3+16, y_pos = 80, y_offset = 32, x_offset = (global.input != inputs.gamepad) ? -8 : -8;
	
	// Draw Controls
	draw_set_font(ft_hud_small);
	draw_set_halign(fa_left);
	draw_text(x_pos+x_offset, y_pos,"Move / Push / Open");
	draw_text(x_pos+x_offset, y_pos+y_offset*1, "Left Hand:\nPick Up / Drop");
	draw_text(x_pos+x_offset, y_pos+y_offset*2, "Right Hand:\nPick Up / Drop");
	draw_text(x_pos+x_offset, y_pos+y_offset*3, "Hold to View Map");
	draw_text(x_pos+x_offset, y_pos+y_offset*4, "Pause / Unpause Game");
	
	x_pos = x_pos - 40;
	if (global.input != inputs.gamepad) {
		x_pos -= 8;
		var input_offset = (global.input == inputs.keyboard_wasd) ? 6 : 0;
		
		draw_sprite_ext(spr_small_key, 0+input_offset ,x_pos, y_pos-8, 1, 1, 1, (game_manager.key_up ? game_color : c_white), 1);
		draw_sprite_ext(spr_small_key, 1+input_offset, x_pos + 16, y_pos+8, 1, 1, 1, (game_manager.key_right ? game_color : c_white), 1);
		draw_sprite_ext(spr_small_key, 2+input_offset, x_pos, y_pos+8, 1, 1, 1, (game_manager.key_down ? game_color : c_white), 1);
		draw_sprite_ext(spr_small_key, 3+input_offset, x_pos - 16, y_pos+8, 1, 1, 1, (game_manager.key_left ? game_color : c_white), 1);
		draw_sprite_ext(spr_small_key, 4+input_offset, x_pos, y_pos+y_offset*1, 1, 1, 1, (game_manager.key_z ? game_color : c_white), 1);
		draw_sprite_ext(spr_small_key, 5+input_offset, x_pos,y_pos+y_offset*2, 1, 1, 1, (game_manager.key_x ? game_color : c_white), 1);
		draw_sprite_ext(spr_large_key, 0, x_pos, y_pos+y_offset*3, 1, 1, 1, (game_manager.key_space ? game_color : c_white), 1);
		draw_sprite_ext(spr_large_key, 1, x_pos,y_pos+y_offset*4, 1, 1, 1, (game_manager.key_enter ? game_color : c_white), 1);
	}
	else {
		x_pos -= 4;
		var d_pad_pressed = game_manager.key_up || game_manager.key_down || game_manager.key_right || game_manager.key_left;
		draw_sprite_ext(spr_dpad, 0,x_pos, y_pos, 1, 1, 0, (d_pad_pressed ? game_color : c_white), 1);
		
		draw_sprite_ext(spr_small_button, 0, x_pos - 14, y_pos+y_offset, 1, 1, 0, (game_manager.key_z ? game_color : c_white), 1);
		draw_sprite_ext(spr_small_button, 4, x_pos - 4, y_pos+y_offset, 1, 1, 0, (game_manager.key_z ? game_color : c_white), 1);
		draw_sprite_ext(spr_large_button, 4, x_pos + 14, y_pos+y_offset, 1, 1, 0, (game_manager.key_z ? game_color : c_white), 1);
		
		draw_sprite_ext(spr_small_button, 1, x_pos - 14, y_pos+y_offset*2, 1, 1, 0, (game_manager.key_x ? game_color : c_white), 1);
		draw_sprite_ext(spr_small_button, 4, x_pos - 4, y_pos+y_offset*2, 1, 1, 0, (game_manager.key_x ? game_color : c_white), 1);
		draw_sprite_ext(spr_large_button, 2, x_pos + 14, y_pos+y_offset*2, 1, 1, 0, (game_manager.key_x ? game_color : c_white), 1);
		
		draw_sprite_ext(spr_small_button, 2, x_pos - 10, y_pos+y_offset*3-8, 1, 1, 0, (game_manager.key_space ? game_color : c_white), 1);
		draw_sprite_ext(spr_small_button, 4, x_pos, y_pos+y_offset*3-8, 1, 1, 0, (game_manager.key_space ? game_color : c_white), 1);
		draw_sprite_ext(spr_small_button, 3, x_pos + 10, y_pos+y_offset*3-8, 1, 1, 0, (game_manager.key_space ? game_color : c_white), 1);
		draw_sprite_ext(spr_large_button, 5, x_pos - 18, y_pos+y_offset*3+8, 1, 1, 0, (game_manager.key_space ? game_color : c_white), 1);
		draw_sprite_ext(spr_small_button, 4, x_pos, y_pos+y_offset*3+8, 1, 1, 0, (game_manager.key_space ? game_color : c_white), 1);
		draw_sprite_ext(spr_large_button, 3, x_pos + 18, y_pos+y_offset*3+8, 1, 1, 0, (game_manager.key_space ? game_color : c_white), 1);
		
		draw_sprite_ext(spr_large_button, 0, x_pos - 18, y_pos+y_offset*4, 1, 1, 0, (game_manager.key_enter ? game_color : c_white), 1);
		draw_sprite_ext(spr_small_button, 4, x_pos, y_pos+y_offset*4, 1, 1, 0, (game_manager.key_enter ? game_color : c_white), 1);
		draw_sprite_ext(spr_large_button, 1, x_pos + 18, y_pos+y_offset*4, 1, 1, 0, (game_manager.key_enter ? game_color : c_white), 1);
	}
}
else if (death_log_screen) { // && (death_count_string != noone || win_count_string != noone)) {
	draw_set_valign(fa_middle);
	draw_set_halign(fa_center);
	draw_set_font(ft_hud);
	title_y_pos = room_height*2;
	
	// Draw Headers
	var y_initial = 16*2, x_initial = (room_width/4)-16, y_pos = y_initial, x_pos = x_initial-16;
	var killed_x_pos = (x_initial*2)-16, deaths_x_pos = (x_initial*3)-8, last_x_pos = (x_initial*4)+16;
	draw_text(room_width/2, 8+15, get_difficulty_string(global.difficulty));
	//draw_text(room_width/2, y_pos-2, get_difficulty_string(global.difficulty));
	y_pos += 16;
	//if (death_count_string != noone) { draw_text(room_width/2, y_pos, death_count_string); }
	//else { draw_text(room_width/2, y_pos, "Death Count: 0"); }
	y_pos += 16;
	draw_set_font(ft_hud_small);
	draw_set_color((death_log_sort == 0) ? game_color : c_white);
	draw_text(killed_x_pos, y_pos+6, "Kills:");
	draw_set_color((death_log_sort == 1) ? game_color : c_white);
	draw_text(deaths_x_pos, y_pos+6, "Deaths:");
	draw_set_color((death_log_sort == 2) ? game_color : c_white);
	draw_text(last_x_pos, y_pos, "Last Run\nEnded:");
	draw_set_font(ft_hud);
	draw_set_color(c_white);
	
	// Draw Arrow Keys
	y_pos += 40;
	if (blink && !game_manager.key_up_pressed && death_log_pos > 0) { draw_sprite_ext(spr_menu_arrow, 0 , x_pos, y_pos-4, 1, 1, 90, c_white, 1); }
	if (blink && !game_manager.key_down_pressed && death_log_pos < array_length(deaths_to_display)-death_types_on_screen) { draw_sprite_ext(spr_menu_arrow, 0, x_pos, room_height-(16*1.5)+4, 1, 1, -90, c_white, 1); }
	y_pos += 8;
	
	// Draw Deaths
	for (var i = 0; i < death_types_on_screen; i++) {
		var log_pos = death_log_pos + i;
		if (log_pos >= array_length(deaths_to_display)) { break; }
		else {
			var death_to_display = deaths_to_display[log_pos], death_obj = death_to_display[0], death_count = death_to_display[1], kill_count = death_to_display[2], last_killed = death_to_display[3] ;
			var last_killed_ordinal = "th", last_killed_final_digit = string_char_at(string(last_killed), string_length(last_killed));
			if (last_killed != 11 && last_killed_final_digit == "1") { last_killed_ordinal = "st"; }
			if (last_killed != 12 && last_killed_final_digit == "2") { last_killed_ordinal = "nd"; }
			if (last_killed != 13 && last_killed_final_digit == "3") { last_killed_ordinal = "rd"; }
			draw_death_type_sprite(x_pos, y_pos, death_obj);
			draw_text(killed_x_pos, y_pos, string(kill_count));
			draw_text(deaths_x_pos, y_pos, string(death_count));
			draw_text(last_x_pos, y_pos, (last_killed == 0) ? "--" : (string(last_killed) + last_killed_ordinal));
			y_pos += 18;
		}
	}
}
else if (evaluation_log_screen) {// && (death_count_string != noone || win_count_string != noone)) {
	draw_set_valign(fa_middle);
	draw_set_halign(fa_center);
	draw_set_font(ft_hud);
	title_y_pos = room_height*2;
	
	// Draw Header
	var y_initial = 16*2, x_initial = (room_width/4)-16, y_pos = y_initial, x_pos = x_initial-16, value_x_pos = room_width-32;
	var best_score_string = get_best_score_string(global.difficulty);
	draw_text(room_width/2, 8+15, get_difficulty_string(global.difficulty));
	
	// Draw Scores
	draw_set_font(ft_hud_small);
	draw_set_color(c_white);
	if (win_count_string != "" || death_count_string != "") { draw_text(room_width/2, room_height-16-12-8, death_count_string+ "; "+win_count_string); }
	if (best_score_string != noone) { draw_text(room_width/2, room_height-16-8, best_score_string); }
	
	// Draw Arrow Keys
	y_pos += 24;
	if (blink && !game_manager.key_up_pressed && evaluation_log_screen > 0) { draw_sprite_ext(spr_menu_arrow, 0 , room_width/2, y_pos-4, 1, 1, 90, c_white, 1); }
	if (blink && !game_manager.key_down_pressed && evaluation_log_screen < array_length(evaluation_manager.evaluation_messages)-8) { draw_sprite_ext(spr_menu_arrow, 0, room_width/2, room_height-(16*3.5)+4, 1, 1, -90, c_white, 1); }
	y_pos += 8;
	
	// Draw Messages
	for (var i = 0; i < 8; i++) {
		var log_pos = evaluation_log_pos + i;
		if (log_pos >= array_length(evaluation_manager.evaluation_messages)) { break; }
		else {
			var message_to_display = evaluation_manager.evaluation_messages[log_pos][0], use_special_color = evaluation_manager.evaluation_messages[log_pos][1];
			var value_to_display = get_evaluation_variable(message_to_display, global.difficulty);
			if (global.is_test_mode) { value_to_display = 9999; }
			if (value_to_display == 0) { message_to_display = "???" }

			draw_set_color((use_special_color) ? game_color : c_white);
			draw_set_halign(fa_left);
			draw_text(x_pos, y_pos,message_to_display);
			draw_set_halign(fa_right);
			if (value_to_display > 0) { draw_text(value_x_pos, y_pos, string(value_to_display)); }
			y_pos += 18;
		}
	}
}
else {
	// Draw Main Menu
	draw_set_valign(fa_middle);
	draw_set_halign(fa_left);
	var title_y_pos = room_height/4-8;
	
	// Draw Game Version
	draw_set_font(ft_hud_small);
	draw_set_halign(fa_center);
	draw_text(room_width/2, room_height-8, "v. " + string(GM_version));
	draw_set_font(ft_hud);
	
	// Draw farmer mode selection
	if (blink && pos == -1) {
		if (global.graphics_mode != graphics_modes.standard) { draw_sprite_ext(spr_title_arrow, 0, room_width/4-32, title_y_pos, 1, 1, 0, c_white, 1); }
		if (global.graphics_mode == graphics_modes.standard || (can_play_unknown_mode() && global.graphics_mode == graphics_modes.farmer)) { draw_sprite_ext(spr_title_arrow, 0, 3*room_width/4+32, title_y_pos, -1, 1, 0, c_white, 1); }
	}
	
	// Draw difficulty selection
	draw_set_halign(fa_center);
	draw_set_font(ft_hud_large);
	var difficulty_y_pos = title_y_pos + room_height/4 - 32 + 8;
	draw_text(room_width/2, difficulty_y_pos, get_difficulty_string(global.difficulty));
	draw_set_font(ft_hud);
	if (blink && pos == 0) {
		if (global.difficulty > difficulties.easy) { draw_sprite_ext(spr_title_arrow, 0, 24, difficulty_y_pos+2, 1, 1, 0, c_white, 1); }
		if (global.difficulty < get_max_difficulty()) { draw_sprite_ext(spr_title_arrow, 0, room_width - 24, difficulty_y_pos+2, -1, 1, 0, c_white, 1); }
	}
	// Draw death count for current difficulty
	if (run_count_string != noone) { draw_text(room_width/2, difficulty_y_pos+18, run_count_string); }
	
	// Draw remaining menu options
	var message_y_pos = difficulty_y_pos+16+24+16, messages_y_offset = -1, message_x_pos = 56;
	draw_set_halign(fa_left);
	draw_set_font(ft_hud);
	draw_text(message_x_pos, message_y_pos+messages_y_offset, get_seed_option_string());
	draw_text(message_x_pos, message_y_pos+messages_y_offset+32, "Options");
	draw_text(message_x_pos, message_y_pos+messages_y_offset+48, "View Controls");
	if (death_count_string != "" || win_count_string != "") { draw_text(message_x_pos, message_y_pos+messages_y_offset+64, + "View Death Log"); }
	if (death_count_string != "" || win_count_string != "") { draw_text(message_x_pos, message_y_pos+messages_y_offset+80, + "View Earned Traits"); }
	var arrow_y =-64;
	if (pos == 1) { arrow_y = message_y_pos; }
	else if (pos == 2) { arrow_y = message_y_pos+16; }
	else if (pos == 3) { arrow_y = message_y_pos+32; }
	else if (pos == 4) { arrow_y = message_y_pos+48; }
	else if (pos == 5) { arrow_y = message_y_pos+64; }
	else if (pos == 6) { arrow_y = message_y_pos+80; }
	
	// Draw seed
	if (global.seed_option == seed_options.specified) {
		draw_set_halign(fa_center);
		draw_text(room_width/2, message_y_pos+messages_y_offset+16, get_zero_padded_string(current_seed, 9)); 
		draw_set_halign(fa_left);
	}
	if (blink) {
		if (pos == 1 && global.is_test_mode) {
			// Draw option arrows
			if (blink) {
				if (global.seed_option > seed_options.rand || (global.seed_option > seed_options.same && global.seed)) { draw_sprite_ext(spr_title_arrow, 0, message_x_pos-16, arrow_y, 1, 1, 0, c_white, 1); }
				if (global.seed_option < seed_options.specified) { draw_sprite_ext(spr_title_arrow, 0, message_x_pos+64, arrow_y, -1, 1, 0, c_white, 1); }
			}
		}
		else if (pos == 2) {
			if (current_seed < MAX_SEED) { draw_sprite_ext(spr_title_arrow, 0, room_width/2 - 40, arrow_y, 1, 1, 0, c_white, 1); }
			if (current_seed > 0) { draw_sprite_ext(spr_title_arrow, 0, room_width/2 + 40, arrow_y, -1, 1, 0, c_white, 1); }
		}
		else { draw_sprite_ext(spr_title_arrow, 0, message_x_pos-16, arrow_y, -1, 1, 0, c_white, 1); }
	}
}


// Draw Logo
draw_set_valign(fa_middle);
draw_set_halign(fa_center);
draw_set_font(ft_hud_giant);
draw_set_color(game_color);
draw_text(room_width/2, title_y_pos, get_graphics_mode_string(global.graphics_mode));
//draw_sprite_ext(spr_logo, global.graphics_mode, room_width/2, title_y_pos, title_scale, title_scale, 0, c_white, 1);

/*
	// Draw seed selection
	if (global.is_test_mode) {
		var seed_y_pos = difficulty_y_pos + 40;
		draw_text(room_width/2, seed_y_pos, get_seed_option_string());
		if (blink && pos == 1) {
			if (global.seed_option > seed_options.rand || (global.seed_option > seed_options.same && global.seed)) { draw_sprite_ext(spr_title_arrow, 0, room_width/4 - 16, seed_y_pos, 1, 1, 0, c_white, 1); }
			if (global.seed_option < seed_options.specified) { draw_sprite_ext(spr_title_arrow, 0, 3*room_width/4 + 16, seed_y_pos, -1, 1, 0, c_white, 1); }
		}
		// Draw seed
		if (global.seed_option == seed_options.specified) { draw_text(room_width/2, seed_y_pos+16, get_zero_padded_string(current_seed, 9)); }
		if (blink && pos == 2) {
			if (current_seed < MAX_SEED) { draw_sprite_ext(spr_title_arrow, 0, 3*room_width/4 + 16, seed_y_pos+16, -1, 1, 0, c_white, 1); }
		    if (current_seed > 0) { draw_sprite_ext(spr_title_arrow, 0, room_width/4 - 16, seed_y_pos+16, 1, 1, 0, c_white, 1); }
		}
	}
	*/